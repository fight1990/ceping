...
@implementation QuickFlashingPresenter
...
+ (void)getLevelDatas:(void(^)(NSArray<MemoryGameModel*> *datas,BOOL isSuccess))resultBlock{
    ...
    [[INetworking shareNet] GET:@"http://114.55.90.93:8081/web/app/flashiconlevelrecordList.json" withParmers:dic do:^(id returnObject, BOOL isSuccess) {
...
+ (void)setLevelParmers:(NSDictionary*)parmers resultBlock:(void(^)(MemoryGameModel *memoryGameModel,BOOL isSuccess))resultBlock {
    ...
    [[INetworking shareNet] GET:@"http://114.55.90.93:8081/web/app/saveflashiconrecords.json" withParmers:dic do:^(id returnObject, BOOL isSuccess) {
        MemoryGameModel *memoryGameModel = nil;
        if (isSuccess) {
            NSDictionary *responseObject = [NSJSONSerialization JSONObjectWithData:returnObject options:NSJSONReadingMutableLeaves error:nil];
            memoryGameModel = [[MemoryGameModel alloc] initWithTrifficLightObject:responseObject];
        }
